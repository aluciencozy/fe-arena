...
4. Configure the backend health checks as `GET /healthz` (process liveness) and `GET /readyz` (question-bank, persistence, and outbox readiness). `/readyz` reports liveness, question-bank readiness, persistence mode, and outbox status separately; question-bank readiness requires all seven published question types, at least one published browser coding fixture, and at least five published noncoding questions for the default five-round playable run. It includes a safe `fallback` persistence mode when Supabase is intentionally omitted and returns HTTP 503 whenever question-bank or persistence readiness is unavailable, or a configured outbox is still starting/degraded; intentional in-memory fallback is also not ready in production.

Solo practice, public queue, and coding-enabled private rounds require cross-origin isolation for the Wasmer SDK. The development-only C practice route uses the same headers when it is enabled locally. The frontend static host must send `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp` on frontend responses, including the HTML, JavaScript, and WebAssembly assets. Vite dev and preview servers set these headers automatically, and `frontend/public/_headers` supplies them for static hosts that support the `_headers` convention. Configure equivalent response headers for other hosts and verify `window.crossOriginIsolated === true` before enabling browser coding traffic. The app also requires a successful browser-worker prewarm before Solo or public queue admission and before a coding-enabled private match starts. Keep cross-origin API and Wasmer registry responses CORS-compatible with the frontend origin.
