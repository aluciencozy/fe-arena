...
4. Configure the backend health checks as `GET /healthz` (process liveness) and `GET /readyz` (question-bank, persistence, and outbox readiness). `/readyz` reports each state separately, including a safe `fallback` persistence mode when Supabase is intentionally omitted, and returns HTTP 503 in production for degraded question-bank or persistence/outbox readiness.
