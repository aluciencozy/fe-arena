...
React/Vite client for the FE Arena unofficial computer science study tool. Run `npm install && npm run dev` from this directory after starting the backend with `cd backend && npm run dev`. Set `VITE_SOCKET_URL` when the Socket.IO backend is not at `http://localhost:3001`; the app reports an actionable connection error if the backend is unavailable.
