...
test("production accepts explicit origins and reports fallback without secrets", () => {
  const config = getRuntimeConfig({
    NODE_ENV: "production",
    PORT: "8080",
    FRONTEND_ORIGINS: "https://arena.example,https://www.example",
    TRUST_PROXY: "true",
  });
...
test("partial Supabase server configuration fails before startup", () => {
  assert.throws(
    () => getRuntimeConfig({ NODE_ENV: "development", SUPABASE_URL: "https://example.supabase.co" }),
    /SUPABASE_URL requires/,
  );
});
