  ...
  rounds: [
    {
      ...
      topicId: "stacks",
      questionBankVersion: "test",
      correctness: { "22222222-2222-4222-8222-222222222222": true },
      responseMs: { "22222222-2222-4222-8222-222222222222": 100 },
      score: { "22222222-2222-4222-8222-222222222222": 1000 },
    },
  ],
});
...
test("history rejects missing and invalid tokens, then returns only the verified user's records", async () => {
  ...
  await repository.persistTerminalMatch(
    snapshot("11111111-1111-4111-8111-111111111111", "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"),
  );
  await repository.persistTerminalMatch(
    snapshot("22222222-2222-4222-8222-222222222222", "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"),
  );
  const verifier: AuthVerifier = {
    verifyAccessToken: async (token) => (token === "valid-one" ? { id: "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" } : null),
  };
  ...
  try {
    ...
    assert.deepEqual(
      body.matches.map((match) => match.matchId),
