...
import { ClientEventSchemas, ServerEventSchemas, type QuestionAttempt } from "../../../shared/domain.js";
...
const safeSeats = (roomId: string): SafeSeat[] =>
  getSeats(roomId).map(({ seatId, name, connected }) => ({ seatId, name, connected }));
...
const makeEvents = (io: Server, roomId: string) => ({
  emit: (state: ReturnType<typeof ensureMatch>) => {
    if (state) io.to(roomId).emit("match:state", output(ServerEventSchemas["match:state"], state));
  },
  message: (text: string) =>
    io
      .to(roomId)
      .emit(
        "chat:message",
        output(ServerEventSchemas["chat:message"], { type: "system", sender: "FE Arena", text, sentAt: Date.now() }),
      ),
});
const error = (socket: Socket, message: string, code = "BAD_REQUEST") =>
  socket.emit("server:error", output(ServerEventSchemas["server:error"], { code, message }));
...
const enterRoom = (
  ...
  socket: Socket,
  roomId: string,
  seat: { seatId: string; name: string; reconnectToken: string },
) => {
  socket.join(roomId);
  socket.emit(
    "room:session",
    output(ServerEventSchemas["room:session"], { roomId, seatId: seat.seatId, reconnectToken: seat.reconnectToken }),
  );
  ensureMatch(roomId);
  emitRoom(io, roomId);
...
const expireDisconnectedSeat = (io: Server, roomId: string, seatId: string, token: string) => {
  ...
  if (state && ACTIVE_PHASES.has(state.phase))
    leaveMatch(roomId, seatId, abandoned ? "abandoned" : "expired", makeEvents(io, roomId));
...
const reserveDisconnectedSeat = (io: Server, roomId: string, seatId: string, token: string) => {
  ...
  reconnectTimers.set(
    token,
    setTimeout(() => expireDisconnectedSeat(io, roomId, seatId, token), 30_000),
  );
};
...
export const registerHandlers = (io: Server, socket: Socket, authVerifier: AuthVerifier) => {
  ...
  socket.on("auth:update", async (payload: unknown) => {
    ...
    if (input.accessToken) {
      try {
        authUserId = (await authVerifier.verifyAccessToken(input.accessToken))?.id;
      } catch {
  ...
  socket.on("room:create-private", (payload: unknown, ack?: (response: PrivateRoomAck) => void) => {
    ...
    socket.emit(
      ...
      output(ServerEventSchemas["room:created"], {
        ...
        reconnectToken: created.seat.reconnectToken,
      }),
    );
  });
  ...
  socket.on("room:join", (payload: unknown) => {
    ...
    if (!result.ok) {
      error(socket, result.error, "ROOM_NOT_FOUND");
      return;
    }
    ...
    io.to(result.metadata.roomId).emit(
      "chat:message",
      output(ServerEventSchemas["chat:message"], {
        type: "system",
        sender: "FE Arena",
        text: `${result.seat.name} joined the study room.`,
        sentAt: Date.now(),
      }),
    );
  });
  ...
  socket.on("queue:join", (payload: unknown) => {
    ...
    if (result.status === "waiting") {
      socket.emit(
        "queue:state",
        output(ServerEventSchemas["queue:state"], {
          status: "waiting",
    ...
    if (!joined.ok) {
      error(socket, "The public match could not be seated. Try again.", "QUEUE_RETRY");
      return;
    }
    ...
    io.to(created.metadata.roomId).emit(
      "queue:matched",
      output(ServerEventSchemas["queue:matched"], { roomId: created.metadata.roomId, metadata: created.metadata }),
    );
    opponentSocket.emit(
      "queue:seat",
      output(ServerEventSchemas["queue:seat"], {
        roomId: created.metadata.roomId,
        seatId: created.seat.seatId,
        reconnectToken: created.seat.reconnectToken,
      }),
    ...
    socket.emit(
      ...
      output(ServerEventSchemas["queue:seat"], {
        ...
        reconnectToken: joined.seat.reconnectToken,
      }),
    );
  });
  socket.on("queue:leave", (payload?: unknown) => {
    if (!validateEmpty(socket, ClientEventSchemas["queue:leave"], payload)) return;
    if (dequeue(socket.id))
      socket.emit("queue:state", output(ServerEventSchemas["queue:state"], { status: "cancelled" }));
  });
  ...
  socket.on("room:reconnect", (payload: unknown) => {
    ...
    if (!result.ok) {
      socket.emit(
        ...
        output(ServerEventSchemas["room:reconnect-failed"], { message: result.error }),
      );
      return;
    }
  ...
  socket.on("match:configure", (payload: unknown) => {
    ...
    if (!input) return;
    if (getMetadata(current.roomId)?.source !== "private")
  ...
  socket.on("chat:send", (payload: unknown) => {
    ...
    if (!input) return;
    const now = Date.now();
    if (now - (chatTimes.get(socket.id) ?? 0) < 1000)
      return error(socket, "Chat is limited to one message per second.", "CHAT_RATE_LIMIT");
    chatTimes.set(socket.id, now);
    io.to(current.roomId).emit(
      "chat:message",
      output(ServerEventSchemas["chat:message"], {
        type: "user",
        sender: current.seat.name,
        text: input.message,
        sentAt: now,
      }),
    );
  ...
  socket.on("solo:submit", (payload: unknown) => {
    ...
    if (!input) return;
    const result = soloSubmit(socket.id, input, (state) =>
  ...
  socket.on("solo:next", (payload?: unknown) => {
    ...
    if (!soloNext(socket.id, (state) => socket.emit("solo:state", output(ServerEventSchemas["solo:state"], state))))
      error(socket, "Finish the current practice result first.", "SOLO_STATE_ERROR");
  });
  ...
  socket.on("room:leave", (payload?: unknown) => {
    ...
    const state = getMatchState(current.roomId);
    if (state && ACTIVE_PHASES.has(state.phase))
      leaveMatch(current.roomId, current.seat.seatId, "forfeit", makeEvents(io, current.roomId));
    const removal = removeSeat(current.roomId, current.seat.seatId);
    socket.leave(current.roomId);
    if (removal) {
      emitRoom(io, current.roomId);
      if (!removal.remaining.length) clearMatch(current.roomId);
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    const state = getMatchState(current.roomId);
    if (state && ACTIVE_PHASES.has(state.phase))
      pauseForDisconnect(current.roomId, current.seat.seatId, makeEvents(io, current.roomId));
    emitRoom(io, current.roomId);
