...
const toRpcPlayers = (snapshot: TerminalMatchSnapshot) =>
...
const toRpcRounds = (snapshot: TerminalMatchSnapshot) =>
...
export class SupabaseMatchRepository implements MatchRepository, AccountHistoryRepository {
  ...
  async persistTerminalMatch(snapshot: TerminalMatchSnapshot): Promise<PersistTerminalResult> {
    ...
    if (data !== "inserted" && data !== "already_exists")
      throw new Error("Supabase terminal match RPC returned an invalid status.");
