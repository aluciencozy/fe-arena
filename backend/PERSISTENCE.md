...
When both values are present, the backend wraps `SupabaseMatchRepository` in a durable local outbox. Snapshots that are staged successfully remain in `.fe-arena-match-outbox` until delivery succeeds and are replayed when the process restarts. Delivery failures use the durable outbox as their sole retry payload. If staging itself fails, the snapshot is retained only in process memory until a later retry can stage it, so it is not durable across a process restart before staging succeeds; [`DEPLOYMENT.md`](../DEPLOYMENT.md) documents how the repository's startup and degraded states affect readiness. `SUPABASE_SECRET_KEY` is a server-only Supabase secret API key: keep it out of frontend environment files, client events, logs, browser bundles, and committed example files. Optional Auth token verification uses backend `SUPABASE_PUBLISHABLE_KEY` when present (with the existing server-only secret as a compatibility fallback). The frontend uses only `VITE_SUPABASE_URL` and `VITE_SUPABASE_PUBLISHABLE_KEY` for Auth; it never receives the secret key.
